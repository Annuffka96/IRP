...
Сценарий: проверка на заполнение справочника Items
	Дано Я открываю навигационную ссылку "e1cib/list/Catalog.Items"
	И таблица "List" содержит строки:
		| 'Code' | 'Description'              |
		| '1'    | 'Товар с характеристиками' |

Сценарий: проверка на добавление штрих-кода (Barcodes) по номенклатуре 
	Дано Я открываю навигационную ссылку "e1cib/list/Catalog.Items"
	...
	Если таблица  "List" не содержит строки: Тогда
		...
		И в поле с именем 'Description' я ввожу текст '123456'
		И я нажимаю на кнопку с именем 'FormWriteAndClose'
	И в таблице "List" я перехожу к строке:
		| 'Serial number' | 'Код' |
		| '123456'        | '1'   |
	И я нажимаю на кнопку с именем 'FormChoose'			
	И я жду закрытия окна 'Item serial/lot number (создание) *' в течение 20 секунд
